...
class LiveVoiceSession:
    ...
    async def start_deepgram_connection(self):
        ...
        try:
            ...
            if self.deepgram_connection.start(options):
                logger.info(f"Deepgram connection started for session {self.session_id}")
                self.is_active = True
                return True
            else:
    ...
    async def on_deepgram_error(self, *args, **kwargs):
        ...
        logger.error(f"Deepgram error for session {self.session_id}: {error}")
        await self.send_to_client({
            "type": "error",
            "message": f"Voice recognition error: {error}"
        })
    ...
    async def send_audio_to_deepgram(self, audio_data: bytes):
        ...
        try:
            if self.deepgram_connection and self.is_active:
                ...
                logger.warning("Deepgram connection not active, cannot send audio")
        except Exception as e:
...
async def health_check():
    ...
    return {
        "status": "OK",
        "service": "AI Tutor Live Voice Service",
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        ...
        if not await session.start_deepgram_connection():
            ...
            try:
                ...
                if message["type"] == "websocket.receive":
                    if "bytes" in message:
                        ...
                        await session.send_audio_to_deepgram(audio_data)
